...
The Heimdall Helm chart separates **non-sensitive configuration** (ConfigMap) from **sensitive credentials** (Secret).
...
::tip
This pattern follows the Vulcan Helm chart and Kubernetes `envFrom` pattern.
::
...
These helpers allow switching between deployment modes:
