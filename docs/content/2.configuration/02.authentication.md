...
The GitHub login button appears when `GITHUB_CLIENTID` is set. Using `GITHUB_CLIENT_ID` (with underscore) will prevent the button from appearing.
::
...
GitLab uses `GITLAB_SECRET` (not `GITLAB_CLIENTSECRET` like other providers). This is a known inconsistency in Heimdall.
::
...
- ❌ `GITHUB_CLIENT_ID` (won't work - two underscores)
