...
::warning
**Phase 2 in Progress** - Bitnami PostgreSQL subchart integration is currently being implemented. This page documents the planned architecture.
::
...
::tip
For production, use a `storageClassName` that provides **persistent volumes** (not `emptyDir`).
::
...
- ✅ Database abstraction helpers in `_helpers.tpl`
- ✅ ConfigMap with database connection details
- ✅ Secrets management (three approaches)
- ✅ External database support in values.yaml

### In Progress (Phase 2)

::warning
These items are **currently being implemented**:
::
...
- ⏳ Health probe endpoint updates

### Remaining Work

- ❌ Standalone PostgreSQL templates (to be **removed**)
  - `postgresql-statefulset.yaml`
  - `postgresql-service.yaml`
  - `postgresql-persistentvolumeclaim.yaml`
