---
title: OAuth/OIDC Validation
description: Comprehensive validation to prevent OAuth/OIDC misconfiguration
navigation:
  icon: i-lucide-shield-check
---

The Heimdall Helm chart implements **3-layer validation** to prevent common OAuth/OIDC configuration mistakes that break authentication.

## Why Validation Matters

OAuth/OIDC configuration is complex and error-prone. Common mistakes include:

- Setting `EXTERNAL_URL` with `/authn` path → **Broken callback URLs**
- Providing Client ID without Client Secret → **Runtime authentication failures**
- Missing OIDC endpoint URLs → **Silent authentication failures**
- Invalid domain formats → **Provider rejection**
...
::tip{title="User-Friendly Errors"}
Schema validation fails **fast** with clear error messages before any Kubernetes resources are created.
::
...
```yaml
{{- if and (hasKey .Values.heimdall.config "GITHUB_CLIENT_ID")
           (not (hasKey .Values.heimdall.secrets "GITHUB_CLIENT_SECRET")) }}
{{- fail "ERROR: GITHUB_CLIENT_ID is set but GITHUB_CLIENT_SECRET is missing." }}
{{- end }}
```
...
heimdall:
  ...
  secrets:
    GITHUB_CLIENT_SECRET: "your-github-client-secret"  # Required!
```
...
- `OKTA_ISSUER`: Pattern `^https://[a-z0-9-]+\.(okta|oktapreview|okta-emea)\.com`
...
The chart includes comprehensive unit tests for all validations:
...
**Error**: `GITHUB_CLIENT_ID is set but GITHUB_CLIENT_SECRET is missing`
...
heimdall:
  ...
  secrets:
    GITHUB_CLIENT_SECRET: "xyz789"  # Add this
```

### OIDC Incomplete Configuration
...
- [Configuration Guide](./03.configuration.md) - Environment variable management
- [Secrets Management](./02.secrets.md) - Three secrets patterns
- [Testing Guide](./13.testing.md) - Running validation tests
